...
def handle_add_node(node_socket, id_to_circuit, circuit_id, last_signal, data, id_to_key, from_socket):
    if circuit_id in id_to_circuit.keys():
        data = decrypt_string(data, [id_to_key[circuit_id]])
        last_signal = decrypt_string(last_signal, [id_to_key[circuit_id]])
        if last_signal == "yes":
            new_node = data.split(" ")
            ...
            add_node_msg = get_add_node_msg(circuit_id, data, [])
